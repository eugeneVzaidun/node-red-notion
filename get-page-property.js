...
module.exports = function (RED) {
  function GetPagePropNode(config) {
    ...
    node.on("input", async function (msg) {
      ...
      try {
        const response = await callHttp(
          `https://api.notion.com/v1/pages/${msg.pageId}/properties/${msg.propertyId}`,
          "GET",
        ...
        if (response.error) {
          ...
          if (response.object === "property_item" || response.object === "list")
            msg.statusCode = 200;
  ...
  RED.nodes.registerType("get page property", GetPagePropNode);
};
